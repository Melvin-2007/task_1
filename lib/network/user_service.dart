...
class UserService {
  ...
  static Future<Either<UserFetchException, UserModel>> fetchUser(String id) async {
    ...
    try {
      ...
      // response status 404
        ...
        return Either.left(UserFetchException(message: model.error!));
      }
      // any other response
      else {
        return Either.left(UserFetchException(message: "Something went wrong"));
      }
    } catch (e) {
      log(e.toString());
      return Either.left(UserFetchException(message: "Something went wrong"));
